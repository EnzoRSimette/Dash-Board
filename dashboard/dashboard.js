window.addEventListener("dados", (e) => {
    ...
    let no_mais_receitas_dados = no_mais_receitas.map(
    ...
    let nos_mais_receitas_dados = nos_mais_receitas.map(
        (item) => item.QUANTIDADE,
    );
    ...
    let tipos_receitas_no_labels = tipos_receitas_no.map(
        (item) => item.NOME_ORGAO,
    ); //* tabela separada
